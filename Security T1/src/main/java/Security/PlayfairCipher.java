...
public class PlayfairCipher {
    ...
    private String prepareText(String text) {
        text = text.toUpperCase().replaceAll("[^A-Z]", "").replace("J", "");
    ...
    public String decrypt(String text) {
        ...
        {
            if (decryptedText.charAt(i) == 'X' && i > 0 && i < (decryptedText.length() -1))
            {
                if (decryptedText.charAt(i - 1) == decryptedText.charAt((i+1)))
                {
                    decryptedText.deleteCharAt(i--);
                }
            }
        }
        if (decryptedText.length() % 2 != 0 && decryptedText.charAt(decryptedText.length() - 1) == 'X')
        {
